...
function deselectAllStickers() {
    if (activeWrapper) {
        // 1. УБИРАЕМ РАМКУ (удаляем класс .active)
        activeWrapper.classList.remove('active');

        // 2. УБИРАЕМ ПУНКТИР (возвращаем прозрачный бордер)
        activeWrapper.style.border = "2px dashed transparent";

        // 3. СКРЫВАЕМ КНОПКИ (удаление, ресайз, поворот)
        activeWrapper.querySelectorAll('.resizer, .sticker-delete').forEach(el => {
            el.style.display = 'none';
        });

        // 4. ОБНУЛЯЕМ ПЕРЕМЕННУЮ
        activeWrapper = null;
    }
}
...
// УНИВЕРСАЛЬНЫЙ СБРОС (ПК + ТЕЛЕФОН)
function handleGlobalDeselect(e) {
    // Если есть активный стикер
    if (activeWrapper) {
        // Проверяем: кликнули ли мы ВНУТРЬ стикера или по КНОПКАМ меню
        const isSticker = e.target.closest('.sticker-wrapper');
        const isMenu = e.target.closest('.sticker-preview-icon') || e.target.closest('.text-color-btn');
        const isControl = e.target.closest('.resizer') || e.target.closest('.sticker-delete');

        // Если клик ВНЕ всего этого — убираем рамку
        if (!isSticker && !isMenu && !isControl) {
            deselectAllStickers();
        }
    }
}

// Слушаем и мышь, и пальцы
document.addEventListener('mousedown', handleGlobalDeselect);
document.addEventListener('touchstart', handleGlobalDeselect, { passive: true });
